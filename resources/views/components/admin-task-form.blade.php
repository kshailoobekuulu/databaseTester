    ...
    @foreach($types as $type)
        <div class="form-control position-relative">
            <input type="radio" name="type" id="{{ $type }}" value="{{ $type }}" class="position-absolute mt-1"
                   required @if($taskType == $type || old('type') == $type) checked @endif>
...
<div class="form-group" id="postgre">
...
<script>
    let oldSelectValues = {
        mysql: '{{ old('mysql_select') ?: ($task ? $task->getMySqlSelect() : '') }}',
        postgre: '{{ old('postgre_select') ?: ($task ? $task->getPostgreSelect() : '') }}',
        mssql: "{{ old('mssql_select')?: ($task ? $task->getMsSqlSelect() : '') }}"
    }
</script>
